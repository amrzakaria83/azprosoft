...
{
    protected $viewPath = 'admin.message';
    private $route = 'admin.messages';
    ...
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            $data = $this->objectModel::where('sender_type', 0)->where('sender_id', auth('admin')->user()->id);
    ...
    {
        $data = $this->objectModel::get();
        $codemp = Auth::guard('admin')->user()->id;
        if ($request->ajax()) {
