...
class EmployeesController extends Controller
{
    public function index(Request $request)
    {
            ...
            return Datatables::of($data)
                ...
                ->addColumn('actions', function($row){
                    $actions = '<div class="ms-2">
                                <a href="'.route('subadmin.employees.show', $row->id).'" class="btn btn-sm btn-icon btn-warning btn-active-dark me-2" data-kt-menu-trigger="click" data-kt-menu-placement="bottom-end">
                                    <i class="bi bi-eye-fill fs-1x"></i>
                                </a>
                                <a href="'.route('subadmin.employees.edit', $row->id).'" class="btn btn-sm btn-icon btn-info btn-active-dark me-2" data-kt-menu-trigger="click" data-kt-menu-placement="bottom-end">
                                    <i class="bi bi-pencil-square fs-1x"></i>
                                </a>
                                <a href="'.route('subadmin.employees.editpass', $row->id).'" class="btn btn-sm btn-icon btn-danger btn-active-dark me-2" data-kt-menu-trigger="click" data-kt-menu-placement="bottom-end">
        ...
        return view('subadmin.employee.index');
    }

    public function create()
    {
        $banks = Bank::get();$jobs = Job::get();$salaries = Salary::get();
        return view('subadmin.employee.create', compact('banks','jobs','salaries'));
    }
    ...
    public function store(Request $request)
    {
        ...
        return redirect('subadmin/employees')->with('message', 'تم الاضافة بنجاح')->with('status', 'success');
    }
    public function show($id)
    {
        $banks = Bank::get();$jobs = Job::get();$salaries = Salary::get();$data = Employee::find($id);
        return view('subadmin.employee.show', compact('banks','jobs','salaries','data'));
    }
    public function edit($id)
    {
        $banks = Bank::get();$jobs = Job::get();$salaries = Salary::get();$data = Employee::find($id);
        return view('subadmin.employee.edit', compact('banks','jobs','salaries','data'));
    }
    ...
    public function update(Request $request)
    {
        ...
        return redirect('subadmin/employees')->with('message', 'تم التعديل بنجاح')->with('status', 'success');
    }
    ...
    public function import () {
        return view('subadmin.employee.import');
    }
    public function importfile (Request $request) {
        ...
        return view('subadmin.employee.importantdata', compact('data'));
    }
    ...
    public function empcheck(Request $request) {
        ...
        if($employee){
            if($employee->is_active == '1'){
                if (Auth::guard('subadmin')->attempt(['name_ar' => $employee->name_ar,
    ...
    public function jobadd(Request $request)
    {
        ...
        $row = Job::create([
            'emp_code' => Auth::guard('subadmin')->id(),
            'name_ar' => $request->name_ar,
            'name_en' => $request->name_en,
            'description' => $request->description,

        ]);
        return redirect('subadmin/employees/create')->with('message', 'تم الاضافة بنجاح')->with('status', 'success');
    }
    ...
    {
        $banks = Bank::get();$jobs = Job::get();$salaries = Salary::get();$data = Employee::find($id);
        return view('subadmin.employee.editpass', compact('banks','jobs','salaries','data'));
    }
