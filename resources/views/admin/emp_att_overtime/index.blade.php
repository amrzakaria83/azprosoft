...
@section('content')
    ...
    <div id="kt_app_content_container" class="app-container container-fluid">
            ...
            <div class="card no-border">
                ...
                <h1 class="text-center text-success fw-bold">{{trans('lang.att_overtime')}}</h1>
                    <div class="row mb-6">
                        
                        <div class="col-sm-4">
                            <label class="required fw-semibold fs-6 mb-2 text-center">{{trans('lang.status')}}</label>
                            <div class="position-relative d-flex align-items-center">
                                <select  class="form-select form-select-lg form-select-solid text-center" id="statusmangeraprove"  name="statusmangeraprove" value="" >
                                    <option value="">{{trans('lang.all')}}</option>
                                    <option value="0">{{trans('lang.waiting')}}-{{trans('lang.approved')}}</option>
                                    <option value="1">{{trans('lang.approved')}}</option>
                                    <option value="2">{{trans('lang.reject')}}</option>
                                    <option value="3">{{trans('lang.delay')}}</option>
                                </select>
                            </div>
                        </div>
                        
                        <div class="col-sm-4">
                            <label class="required fw-semibold fs-6 mb-2 text-center">{{trans('lang.start_from')}}</label>
                            <div class="position-relative d-flex align-items-center">
                        ...
                        <div class="col-sm-4">
                            <label class="required fw-semibold fs-6 mb-2 text-center">{{trans('lang.end_to')}}</label>
                            <div class="position-relative d-flex align-items-center">
                                <span class="svg-icon svg-icon-2 position-absolute mx-4">
                                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <path opacity="0.3" d="M21 22H3C2.4 22 2 21.6 2 21V5C2 4.4 2.4 4 3 4H21C21.6 4 22 4.4 22 5V21C22 21.6 21.6 22 21 22Z" fill="currentColor" />
                    ...
                    <table class="table align-middle table-rounded table-striped table-row-dashed fs-6" id="kt_datatable_table">
                        ...
                        <thead class="bg-light-dark pe-3">
                            ...
                            <tr class="text-center text-dark fw-bold fs-4 text-uppercase gs-0">
                                ...
                                <th class="min-w-125px text-start text-center">{{trans('lang.duration')}}</th>
                                <th class="min-w-125px text-start text-center">{{trans('lang.start_from')}}</th>
                                <th class="min-w-125px text-start text-center">{{trans('lang.end_to')}}</th>
                                <th class="min-w-125px text-center">{{trans('lang.type_type')}}</th>
                                <th class="min-w-125px text-center">{{trans('lang.director')}}</th>
...
$("#kt_datepicker_1").flatpickr({
    defaultDate: new Date().setDate(new Date().getDate() - 30), // Previous 2 days
    enableTime: true,
    allowInput: true,
...
<script>
    ...
    $(function () {
        ...
        var table = $('#kt_datatable_table').DataTable({
            ...
            ajax: {
                ...
                data: function (d) {
                    ...
                    d.statusmangeraprove = $('#statusmangeraprove').val(),
                    d.search = $('#search').val()
                }
            },
            columns: [
                ...
                {data: 'attendance_out_from', name: 'attendance_out_from'},
                {data: 'attendance_in_to', name: 'attendance_in_to'},
                {data: 'type_emp_att_request', name: 'type_emp_att_request'},
                {data: 'noterequest', name: 'noterequest'},
                {data: 'note', name: 'note'},
        ...
        $('#kt_datepicker_1,#kt_datepicker_2,#statusmangeraprove').change(function() {
            table.draw();
        });
