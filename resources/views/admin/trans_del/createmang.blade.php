    ...
    <div id="kt_app_content_container" class="app-container container-fluid">       
        ...
        <div class="card mb-5 mb-xl-10">
            ...
            <div id="kt_account_settings_profile_details" class="collapse show">
                <form action="{{route('admin.trans_dels.storemang')}}" method="POST" enctype="multipart/form-data" id="kt_account_profile_details_form" class="form">
                    @csrf
                    ...
                    <div class="card no-border">
                        <div class="row">
                            ...
                            <div class="col-8 border border-end">
                                ...
                                <div class="row mb-6">
                                        ...
                                        <div class="col-lg-4 fv-row">
                                            <select  data-placeholder="Select an option" class=" input-text form-control form-select mb-3 mb-lg-0" id="del_code"  name="del_code" data-control="select2" >
                                        ...
                                        <!-- <label class="col-sm-2 col-form-label fw-semibold fs-6">طبيعة</label> -->
                                        <div class="col-sm-3 d-flex align-items-center">
                                        ...
                                        <div class="card-footer card-footer-sm d-flex justify-content py-6 px-9">
                                            <div class="col-4">
                                                    <div class=" d-flex justify-content-start py-6 px-9" id="gonodel">
                                                            <a href="" id="addedreq" class="btn btn-lg  btn-danger btn-active-success  align-middle container ">
                                                            <span>بدون طيار</span>
                                            ...
                                            <div class="col-4" style="display: none;">
                                                <div class="d-flex justify-content-start py-6 px-9" id="godel" >
                                ...
                                <div class="col-4">
                                    <span>طلبات</span>
                                    <table class="table align-middle table-rounded table-striped table-row-dashed fs-6" id="kt_datatable_table2">
                                        <!--begin::Table head-->
                                        <thead class="bg-light-dark pe-3">
                                            <!--begin::Table row-->
                                            <tr class="text-start text-dark fw-bold fs-4 text-uppercase gs-0">
                                                <th class="w-10px p-3">
                                                    <div class="form-check form-check-sm form-check-custom form-check-solid ">
                                                        <input class="form-check-input" type="checkbox" data-kt-check="true" data-kt-check-target="#kt_datatable_table2 .form-check-input" value="1" />
                                                    </div>
                                                </th>
                                                <th class="min-w-125px text-start">من</th>
                                                <th class="min-w-125px text-start">الحركة</th>
                                            </tr>
                                            <!--end::Table row-->
                                        </thead>
...
<script>
    $(function () {
        ...
        var table = $('#kt_datatable_table').DataTable({
            ...
            ajax: {
                url: "{{ route('admin.trans_dels.index') }}",
                data: function (d) {
...
<script>
    $(function () {
        ...
        var table = $('#kt_datatable_table1').DataTable({
            ...
            ajax: {
                url: "{{ route('admin.trans_dels.indexmang') }}",
                data: function (d) {
                    d.is_active = $('#is_active').val(),
                    d.search = $('#search').val()
                }
...
<script>
    $(function () {
        ...
        var table = $('#kt_datatable_table2').DataTable({
            processing: false,
            serverSide: true,
            searching: false,
            autoWidth: false,
            ...
            ajax: {
                url: "{{ route('admin.trans_dels.indexreq') }}",
                data: function (d) {
                    d.is_active = $('#is_active').val(),
                    d.search = $('#search').val()
                }
...
<script>
    function edit_status(id) {
        Swal.fire({
            ...
            '  <option selected value="1">استلام</option>' +
            ...
            $.ajax({
                url: "{{ url('/admin/trans_dels/edit_status') }}/" + id + "/" + status_edit,
                method: "GET",
...
<script>
    function edit_to_id(id) {
        Swal.fire({
            html: `
                <select class="form-control" id="edit_to_id">
            ...
            if (result.value) {
            var edit_to_id = document.getElementById('edit_to_id').value;

            $.ajax({
                url: "{{ url('/admin/trans_dels/edit_to_id') }}/" + id + "/" + edit_to_id,
                method: "GET",
...
<script>
    function edit_del(id) {
        Swal.fire({
            html: `
                <select class="form-control form-select select2" id="edit_del" data-control="select2">
                    @foreach ($dataemp as $emp)
                    <option value="{{ $emp->id }}">{{ $emp->name_ar }}</option>
                    @endforeach
                </select>
                `,
            title: ' تسكين طيار',
            ...
            if (result.value) {
            var edit_del = document.getElementById('edit_del').value;

            $.ajax({
                url: "{{ url('/admin/trans_dels/edit_del') }}/" + id + "/" + edit_del,
                method: "GET",
