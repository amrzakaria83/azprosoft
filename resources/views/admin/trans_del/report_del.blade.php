            ...
            <div class="card no-border">
                <div class="row align-items-start">
                    ...
                    <div class="row mb-6">
                            <label class="col-lg-2 col-form-label fw-semibold fs-6">
                                <span class="required">اختر الطيار</span>
                                <div class=" d-flex justify-content-start py-6 px-9">
                                    <a href="" id="added" class="btn btn-lg  btn-success btn-active-dark  align-middle container ">
                                    <span>بحث</span>
                            ...
                            <div class="col-sm-4 d-flex align-items-center text-center">
                                <select data-placeholder="Select an option" class="form-select text-center  model_id" autofocus required aria-label="Select example" id="model_id" name="model_id" data-control="select2">
                                    <option></option>
                                    @if(isset($data))
                                        @foreach ($data as $asd)
                            ...
                            <div class="col-sm-2">
                                <label class="required fw-semibold fs-6 mb-2 text-center">تاريخ  من</label>
                            ...
                            <div class="col-sm-2">
                                <label class="required fw-semibold fs-6 mb-2 text-center">تاريخ  الى</label>
                                <div class="position-relative d-flex align-items-center">
                                    <span class="svg-icon svg-icon-2 position-absolute mx-4">
                                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                            <path opacity="0.3" d="M21 22H3C2.4 22 2 21.6 2 21V5C2 4.4 2.4 4 3 4H21C21.6 4 22 4.4 22 5V21C22 21.6 21.6 22 21 22Z" fill="currentColor" />
                ...
                <div class="row">
                    <div class="justify-content-center">
                        ...
                        <span class="text-success fs-1">اسم الطيار  : </span><span class="text-danger fs-1 fw-bold" id="dataname"></span><br>
                            <span class="text-primary fs-1">من </span><span class="text-info fs-1" id="fromdatecarb" ></span>
                            <span class="text-primary fs-1">الى </span><span class="text-info fs-1" id="todatecarb" ></span>
                        <br>
                    ...
                    <div class="row align-items-start">
                        <div class="col-sm-12 p-3">
                            <h1>
                                عدد التحويلات <span class="text-success" id="total_trans" ></span> - 
                                عدد الاوردرات <span class="text-success" id="total_order" ></span> - 
                                عدد التحويلات الملغية <span class="text-danger" id="total_canceltrans" ></span> - 
                                عدد الاوردرات الملغية <span class="text-danger" id="total_cancelorder" ></span>
                            </h1>
                        </div>
...
<script>
    ...
    defaultDate: new Date(new Date().setDate(0)), // Last day of the previous month
    enableTime: true,
    allowInput: true,
...
<script>
    ...
    $('#added').click(function(e) {
        e.preventDefault();
        const del_code = $(".model_id option:selected").val();
        const fromdate = $("#kt_datepicker_1").val();
        const todate = $("#kt_datepicker_2").val();

        $.ajax({
            type: "GET",
            url: "{{ route('admin.trans_dels.indexdel_view') }}" + '/' + del_code + '/' + fromdate + '/' + todate,
            success: function(response) {
                // Handle the response data here
                console.log(response);
                $('#dataname').text(response.dataname.name_ar);
                $('#fromdatecarb').text(response.fromdatecarb);
                $('#todatecarb').text(response.todatecarb);
                $('#total_trans').text(response.total_trans);
                $('#total_order').text(response.total_order);
                $('#total_cancelorder').text(response.total_cancelorder);
                $('#total_canceltrans').text(response.total_canceltrans);
      ...
      var table = $('#kt_datatable_table').DataTable({
          ...
          ajax: {
              url: "{{ route('admin.trans_dels.indexdelreport') }}",
              data: function (d) {
                  d.is_active = $('#is_active').val(),
                  d.from_time = $('#kt_datepicker_1').val(),
                  d.to_time = $('#kt_datepicker_2').val(),
                  d.del_code = del_code,
                  d.search = $('#search').val()
              }
          },
          columns: [
              ...
              {data: 'note', name: 'note'},
              {data: 'empreturne', name: 'empreturne'},
      ...
      //         table.draw();
      //     });
    //   $('#kt_datepicker_1,#kt_datepicker_2').change(function() {
