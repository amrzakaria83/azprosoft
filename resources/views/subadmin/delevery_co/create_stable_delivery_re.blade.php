    ...
    <div data-kt-swapper="true" data-kt-swapper-mode="prepend" data-kt-swapper-parent="{default: '#kt_content_container', 'lg': '#kt_header_nav'}" class="page-title d-flex align-items-center flex-wrap me-3 mb-5 mb-lg-0">
        ...
        <ul class="breadcrumb breadcrumb-separatorless fw-bold fs-7 my-1">
            <li class="breadcrumb-item text-muted px-2">
                <a  href="{{ route('subadmin.delevery_cos.index') }}" class="text-muted text-hover-primary fw-bold fs-2 ">طلبات التوصيل</a>
            </li>
        </ul>
    </div>
    <!--end::Page title-->
    ...
    $(function () {
        ...
        $('#asdasd').click(function(){
            ...
            if (del_code) {
                ...
                $.ajax({url: "{{url('/')}}"+"/subadmin/delevery_cos/storestable?id="+id+"&time_go_request="+theTime+"&del_code="+is_del_code, success: function(result){
                if (del_code) {
                    table.draw();
                    location.reload();
                } else {
    ...
    function customer_check_stable(id, value_return) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    url: "{{ url('/subadmin/delevery_cos/edit_value_return_stable_no_total') }}/" + id + "/" + value_return,
                    method: "GET",
                    success: function (result) {
                        location.reload();
                        var del_code = $('#delivery_name_order').val();
                $.ajax({
                url: "{{ route('subadmin.delevery_cos.get_totalorder_request') }}",
                method: 'GET',
    ...
    function customer_check_stable_change_total(id, totalValue) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    url: "{{ url('/subadmin/delevery_cos/edit_value_return_stable_total') }}/" + id + "/" + totalValue,
                    method: "GET",
                    success: function (result) {
                        location.reload();
                        Swal.fire('تم التعديل بنجاح', '', 'success');
