                            ...
                            <div class="row align-items-start">
                                <div class="col-sm-6 p-3">
                                        <h3 style="width: 70%">طلب توصيل
                                            <li class=" text-muted px-2">
                                                <a  href="{{ route('subadmin.delivery_customers.indexcutomer_del_all_data') }}" class="text-warning text-hover-primary fw-bold fs-2 ">- العملاء-</a>
                                                {{-- <a  href="{{ route('subadmin.delevery_cos.total_del_report') }}" class="text-info text-hover-primary fw-bold fs-2 ">- شغل مجمع-</a> --}}
                                                {{-- <a  href="{{ route('subadmin.delevery_cos.get_report_data') }}" class="text-warning text-hover-primary  fs-5 ">-شغل مجمع تفصيلى-</a> --}}
                                                <a  href="{{ route('subadmin.delevery_cos.indexcustomer_order_request') }}" class="text-primary text-hover-primary  fs-5 ">-طلبات عميل تفصيلى-</a>
                                                {{-- <a  href="{{route("subadmin.delevery_cos.delivery_order_requests_stable_payment")}}" class="text-info text-hover-primary fw-bold fs-2 ">حساب طيار-</a>
                                                <a  href="{{route("subadmin.delevery_cos.del_report_index_notsupply")}}" class="text-danger text-hover-primary fw-bold fs-2 ">لم يتم التوريد</a>
                                                <a  href="{{route("subadmin.delevery_cos.del_report_index")}}" class="text-success text-hover-primary fw-bold fs-2 ">-شغل طيار</a> --}}
                                            </li>
                                        </h3>
                                            <!--begin::Card body-->
                                            <div class="card-body py-4">
                                ...
                                <div class="col-sm-6 p-3">
                                    <h3>الطياريين المتاحين
                                        ...
                                        <li class=" text-muted px-2">
                                            <a  href="{{ route('subadmin.delevery_cos.delivery_stable_requests_add_view') }}" class="text-muted text-hover-primary fw-bold fs-2 ">-طلبات تثبيت-</a>
                                            <a  href="{{route("subadmin.delevery_cos.delivery_order_requests_stable_payment")}}" class="text-info text-hover-primary fw-bold fs-2 ">حساب طيار-</a>
                                            <!--<a  href="{{route("subadmin.delevery_cos.del_report_index_notsupply")}}" class="text-danger text-hover-primary fw-bold fs-2 ">لم يتم التوريد</a>-->
                                            <a  href="{{route("subadmin.delevery_cos.del_report_index_notsupply_delay")}}" class="text-danger text-hover-primary fw-bold fs-2 ">-آجل لم تحصل</a>
                                            <a  href="{{route("subadmin.delevery_cos.delivery_order_requests_stable__delay_payment")}}" class="text-success text-hover-primary fw-bold fs-2 ">-حساب عميل</a>
                                        </li>
                                    </h3>
                                        <!--begin::Card body-->
                                        <div class="card-body py-4">
...
<script>
        ...
        $(function () {
            ...
            $('.delivery_name_order').select2({
                    ...
                    ajax: {
                    type: "GET",
                    url: "{{ url('subadmin/delevery_cos/getdelselect')}}",
                    dataType: 'json',
                    delay: 250,
                    processResults: function (data) {
                        return {
            ...
            $('#submitForm_add_order_req').click(function(){
                ...
                $.ajax({
                    url: "{{url('/')}}"+"/subadmin/delevery_cos/store?id="+id+"&del_code="+is_del_code+"&note="+note+"&moove_time="+moove_time,
            ...
            $("#btn_delete").click(function(event){event.preventDefault();
                var checkIDs = $("#kt_datatable_table_delevry_order_request input:checkbox:checked").map(function(){return $(this).val();
                }).get(); // <----
...
<script>
    $(function () {
        ...
        $("#btn_delete").click(function(event){event.preventDefault();
            ...
            var checkIDs = $("#kt_datatable_table_delivery_out input:checkbox:checked").map(function(){return $(this).val();
            }).get(); // <----
...
<script>
    ...
    if (checkIDs.length > 0) {var token = $(this).data("token");
    Swal.fire({title: 'هل انت متأكد ؟',text: "لا يمكن استرجاع البيانات المحذوفه",
    type: 'warning',
    showCancelButton: true,
    confirmButtonClass: 'btn btn-success',
    cancelButtonClass: 'btn btn-danger m-l-10',
    confirmButtonText: 'موافق',cancelButtonText: 'لا'}).then(function (isConfirm) {
        if (isConfirm.value) {
            $.ajax({
                url: "{{route('subadmin.drug_requests.delete')}}",
        type: 'post',
        dataType: "JSON",
        ...
        success: function (data) {if(data.message == "success") {
            table.draw();
            toastr.success("", "تم الحذف بنجاح");
        } else {toastr.success("", "عفوا لم يتم الحذف");
...
<script>
    fetch('subadmin/get_cashiar_value')
