    ...
    <div data-kt-swapper="true" data-kt-swapper-mode="prepend" data-kt-swapper-parent="{default: '#kt_content_container', 'lg': '#kt_header_nav'}" class="page-title d-flex align-items-center flex-wrap me-3 mb-5 mb-lg-0">
        ...
        <ul class="breadcrumb breadcrumb-separatorless fw-bold fs-7 my-1">
            <li class="breadcrumb-item text-muted px-2">
                <a  href="" class="text-muted text-hover-primary">'الاستعلام عن دليقرى'</a>
            </li>
        </ul>
    </div>
    <!--end::Page title-->
...
@section('content')
    ...
    <div id="kt_app_content_container" class="app-container container-fluid">
            ...
            <div class="card no-border">
                ...
                <div class="card-header border-0 pt-6">
                    <div class="card-title">
                        <!--begin::Search-->
                            <input type="hidden" data-kt-db-table-filter="search" id="search" name="search" class="form-control form-control-solid bg-light-dark text-dark w-250px ps-14" placeholder="Search.." />
                    </div>
                    <div class="card-toolbar">
                        <!--begin::Toolbar-->
                        <div class="d-flex justify-content-end dbuttons">
                            <a href="{{route('subadmin.order_ph_requests.create')}}" class="btn btn-sm btn-icon btn-primary btn-active-dark me-3 p-3">
            ...
            <div class="row">
                    ...
                    <div class="col-sm-2">
                        <label for="toit">الحالة</label>
                                <select class="form-select" autofocus required aria-label="Select example" id="status" name="status" >
                                    <option value="">الجميع</option>
                                        <option value="2">تم التسليم الاوردر</option>
                                        <option value="1">تم الغاء الاوردر</option>
                                </select>
                    </div>
                </div>
                <div class="row">
                ...
                <div class="card-body py-4">
                    ...
                    <table class="table align-middle table-rounded table-striped table-row-dashed fs-6  " id="kt_datatable_table">
                        ...
                        <thead class="bg-light-dark pe-1">
                            ...
                            <th class="min-w-10px text-start"> مدة التنفيذ</th>
                            <th class="min-w-10px text-start">الحالة</th>
                            <th class="min-w-10px text-start">وقت التحرك</th>
                            <th class="min-w-10px text-start">وقت الرد</th>
...
$('.name_ar').select2({
            ...
            ajax: {
                type: "GET",
                url: "{{ url('subadmin/order_ph_requests/getempdel') }}",
                dataType: 'json',
                delay: 250,
    ...
    var CSRF_TOKEN = $('meta[name="csrf-token"]').attr('content');
        ...
        $("#no_recepient").on('blur', function() {
        var no_recepient = $(this).val();});
...
<script>
    var table = $('#kt_datatable_table').DataTable({
            ...
            ajax: {
                url: "{{ route('subadmin.order_ph_requests.indexdone') }}",
                data: function (d) {
                    d.toit = $('#toit').find(":selected").val(),
                    d.del_code = $('#del_code').val(),
                    d.status = $('#status').val(),
                    d.no_recepient = $('#no_recepient').val()
        ...
        $('#kt_datepicker_1, #toit, #del_code, #no_recepient, #status, #kt_datepicker_2').change(function() {
            table.draw();
        });
